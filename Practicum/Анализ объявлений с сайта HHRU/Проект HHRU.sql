-- Получим статистики по заработной плате
...
-- Выявим грейд требуемых специалистов по опыту
SELECT experience,
       COUNT(*) AS num_vacancies
FROM public.parcing_table 
GROUP BY experience 
...
-- Определеним доли грейдов среди вакансий аналитиков

-- Чтобы это сделать, нам понадобится два запроса:

-- 1) Сначала вычислим общее количество вакансий вручную:
...
-- Определим типичное места работы для аналитиков по различным параметрам
SELECT employer,
       COUNT(*) AS num_vacancies,
...
-- Частота упоминания hard и soft skills (пример для key_skills_1)
SELECT key_skills_1,
